...
def _render_ai_disclaimer() -> None:
    ...
    <div class="sr-disclaimer">
        ...
        <p><strong>{t('ai_note_warning')}</strong></p>
    </div>
...
def render_ai_analysis_panel(review_id: int) -> None:
    ...
    c1.metric(t("total_articles_ai"), n_total)
    c2.metric(t("included_ai"),       n_incl)
    c3.metric(t("pending_ai"),        n_pending)
    c4.metric(t("conflicts_ai"),      n_conflict)

    if n_pending > 0:
        st.warning(f"⚠️ **{n_pending}** {t('pending_screening')}")
    if n_conflict > 0:
        st.warning(f"⚠️ **{n_conflict}** {t('conflict_unresolved')}")

    if n_incl == 0:
        st.info(t("no_articles_included"))
        return
    ...
    <div style="display:flex;align-items:center;gap:0.75rem;
        ...
        <div>
            ...
            {t("included_ai_count").format(n=n_incl)}
            </span>
...
def _tab_data_pool(review_id, articles):
    st.markdown(f'''<div class="sr-section-header">
      <div class="sr-section-icon" style="background:#F5F3FF;">📊</div>
      <div><h3 style="color:var(--ink,#0F172A);">{t("data_pooling_title")}</h3>
           <p style="font-size:0.78rem;color:var(--ink-40,#64748B);margin:0;">
           {t("data_pooling_desc")}</p>
      </div></div>''', unsafe_allow_html=True)
    _render_ai_disclaimer()

    with st.expander(t("fields_extracted"), expanded=False):
        st.markdown(
    ...
    run_all = c1.button(t("bulk_extract_all"), key="pool_all", type="primary")
    if c2.button(t("btn_clear_retry"), key="pool_clear"):
    ...
    for article in articles:
        ...
        with st.expander(f"**{title[:80]}**", expanded=False):
            ...
            if not abstract:
                st.warning(t("no_abstract_warning"))

            run_one = st.button(t("btn_extract_one"), key=f"pool_{pmid}")
            ...
            if sd:
                ...
                if sentences:
                    with st.expander(t("source_sentences"), expanded=False):
    ...
    if all_sd:
        st.divider()
        st.markdown(f"#### {t('composite_table').format(n=len(all_sd), total=len(articles))}")
        ...
        # ── Forest Plot Preview ────────────────────────────────────────
        st.divider()
        st.markdown(f"#### {t('forest_plot')}")
        st.caption(t("forest_plot_desc"))
...
def _pool_summary(df):
    ...
    if "Primary Result" in df.columns:
        ...
        c2.metric(t("outcomes_reported"), f"{reported}/{len(df)}")
    if "Comparator" in df.columns:
        c2.metric(t("comparator_avail"),
...
def _tab_risk_of_bias(review_id, articles):
    st.markdown(f'''<div class="sr-section-header">
      ...
      <div><h3 style="color:var(--ink,#0F172A);">{t("rob_title")}</h3>
      </div></div>''', unsafe_allow_html=True)
    st.caption(t("rob_desc"))
    _render_ai_disclaimer()

    with st.expander(t("how_to_interpret"), expanded=False):
        st.markdown(f"""
{t("rob2_explain")}
...
""")
    ...
    c1, c2 = st.columns([2, 1])
    run_all = c1.button(t("btn_assess_all"), key=f"rob_all_{review_id}", type="primary")
    if c2.button(t("btn_clear_rob"), key=f"rob_clear_{review_id}"):
        for a in articles:
            for _task in ("study_class", "rob2", "nos"):
                k = _ck(review_id, a["pmid"], _task)
    ...
    for article in articles:
        ...
        with st.expander(f"{icon} **{title[:80]}**", expanded=False):
            ...
            run_one = st.button(t("btn_assess_article"), key=f"rob_{pmid}")
            ...
            if clf:
                ...
                c_des.markdown(
                    f"**{t('study_design')}** {clf.emoji} {clf.display_design}"
                )
                conf_colour = {"high": "🟢", "moderate": "🟡", "low": "🔴"}.get(
                    clf.confidence, "⬜"
                )
                c_conf.markdown(
                    f"**{t('confidence')}** {conf_colour} {clf.confidence.capitalize()}"
                )
                if clf.signals:
                    st.caption(t("signals") + " · ".join(clf.signals[:3]))
    ...
    if rob_rows or nos_rows:
        st.divider()
        st.markdown(t("traffic_light_header"))
        st.caption(t("cochrane_visual"))
...
def _render_traffic_light(review_id, articles):
    ...
    for label, color, sym in [
        (t("low_risk_good"),      "#2E7D32", "+"),
        (t("some_concerns_fair"), "#F9A825", "?"),
        (t("high_risk_poor"),     "#C62828", "−"),
        (t("not_assessed"),       "#E0E0E0", ""),
    ]:
        fig.add_trace(go.Scatter(
            x=[None], y=[None],
            mode="markers",
...
def _render_rob2(rob2) -> None:
    ...
    with st.expander(t("signalling_detail"), expanded=False):
        for dom in rob2.domains:
...
def _tab_grade(review_id, articles):
    ...
    st.markdown(f'''<div class="sr-section-header">
      ...
      <div><h3 style="color:var(--ink,#0F172A);">{t("grade_certainty_title")}</h3>
      </div></div>''', unsafe_allow_html=True)
    st.caption(t("grade_caption"))
    _render_ai_disclaimer()

    with st.expander(t("grade_title"), expanded=False):
        st.markdown("""
...
""")
    ...
    st.info(
        f"📊 **{t('study_design_breakdown')}** "
        f"{t('rcts_label')}: **{len(rct_articles)}** · "
        f"{t('observational_label')}: **{len(obs_articles)}** · "
        f"{t('unclassified_label')}: **{len(unclassified)}**"
    )

    if unclassified:
        _DEST_KEYS = ["RCTs", "Observational studies", "Exclude from GRADE"]
        _DEST_TK   = ["rcts_label", "observational_label", "exclude_from_grade"]
        _dest_idx  = st.radio(
            t("assign_unclassified"),
            options=range(len(_DEST_KEYS)),
            format_func=lambda i: t(_DEST_TK[i]),
            horizontal=True, key=f"grade_unclassified_{review_id}",
        )
        dest = _DEST_KEYS[_dest_idx]
        if dest == "RCTs":                   rct_articles += unclassified
        elif dest == "Observational studies": obs_articles += unclassified

    # ── Outcome manager ────────────────────────────────────────────────────────
    st.divider()
    st.markdown(f"#### {t('step1_outcomes')}")
    ...
    if st.button(t("btn_add_outcome"), key=f"oadd_{review_id}"):
    ...
    with col_ext:
        if st.button(t("btn_extract_all_outcomes"),
    ...
    with col_clr2:
        if st.button(t("btn_clear_grade"), key="grade_clear"):
    ...
    st.divider()
    st.markdown(f"#### {t('sof_title')}")
    st.caption(t("sof_caption"))
    ...
    # ── Manual event counts ────────────────────────────────────────────────────
    _manual_ev = {}
    with st.expander(t("manual_event_entry"), expanded=False):
        st.caption(t("manual_event_format"))
        for out in outcomes:
            oname = out["name"]
            dmap  = grade_results.get(oname, {})
...
<style>
  ...
  .certainty-very_low {{ color:#B71C1C; font-weight:bold; }}
    ...
    st.divider()
    st.markdown(t("detailed_breakdown"))
    ...
    for out in outcomes:
        ...
        for design_key, design_label in [
            ("rct", t("design_rcts")), ("observational", t("design_obs"))
        ]:
            ...
            with st.expander(
                ...
                st.caption(f"{t('rationale_prefix')} {grade.certainty_rationale}")

                mc1, mc2, mc3, mc4 = st.columns(4)
                mc1.metric(t("grade_studies"),      grade.n_studies)
                mc2.metric(t("grade_participants"), grade.n_participants or "NR")
                mc3.metric(t("grade_starting"),     grade.starting_certainty.upper())
                mc4.metric(t("grade_final"),
                           grade.final_certainty.replace("_"," ").upper())

                st.markdown(t("downgrade_upgrade"))
                ...
                with st.expander(t("factor_rationales"), expanded=False):
                    for f in grade.all_factors:
                        if f.direction != "none" and f.rationale:
                            st.markdown(
                                f"**{f.name}** — "
                                f"{'↓'*f.levels} {t('downgrade_label')}"
                                if f.direction=="downgrade"
                                else f"**{f.name}** — ↑ {t('upgrade_label')}")
                            st.caption(f"  {f.rationale}")

                # Reporting template
                if design_key == "rct":
                    _study_word = t("grade_randomised_trial") if grade.n_studies == 1 \
                                  else t("grade_randomised_trial") + "s"
                else:
                    _study_word = t("grade_obs_study") if grade.n_studies == 1 \
                                  else t("grade_obs_studies")
                n_word = f"{grade.n_studies} {_study_word}"
                st.divider()
                st.markdown(t("reporting_template"))
                st.code(
                    t("grade_certainty_sentence").format(
                        outcome=grade.outcome,
                        n_word=n_word,
                        n=grade.n_participants or "NR",
                        certainty=grade.final_certainty.replace("_"," "),
                        display=CERTAINTY_DISPLAY.get(grade.final_certainty,""),
                        rationale=grade.certainty_rationale,
                    ),
                    language=None)
...
def _build_sof_html_multi(grade_results: dict,
    ...
    def _fmt(factor) -> str:
        if factor.direction == "downgrade":
            return _t("sof_serious") if factor.levels==1 else _t("sof_very_serious")
        if factor.direction == "upgrade": return _t("sof_upgrade")
        return _t("sof_not_serious")
...
<tr>
  <th rowspan="2" style="text-align:left;min-width:160px;">
    {_t("sof_col_outcome")}</th>
  <th rowspan="2">{_t("sof_col_design")}</th>
  <th colspan="5" style="background:#2C5F8A;">{_t("sof_cert_assessment")}</th>
  <th colspan="2" style="background:#2C5F8A;">{_t("sof_event_rates")}</th>
  <th rowspan="2">{_t("sof_col_effect")}</th>
  <th rowspan="2">{_t("sof_col_certainty")}</th>
</tr>
<tr>
  <th>{_t("sof_col_rob")}</th>
  <th>{_t("sof_col_inconsistency")}</th>
  <th>{_t("sof_col_indirectness")}</th>
  <th>{_t("sof_col_imprecision")}</th>
  <th>{_t("sof_col_pub_bias")}</th>
  <th>{_t("sof_col_intervention")}</th>
  <th>{_t("sof_col_comparator")}</th>
</tr>
</thead>
<tbody>
"""
...
"""
        ...
        for design_key, design_label in [("rct","RCTs"),
            ...
            _d_label = _t("design_rcts") if design_key=="rct" else _t("design_obs")
            _s_word  = _t("grade_obs_study") if g.n_studies == 1 else _t("grade_obs_studies")
            design_cell = (
                f"<b>{_d_label}</b><br>"
                f"1 {_s_word}" if g.n_studies==1
                else
                f"<b>{_d_label}</b><br>"
                f"{g.n_studies} {_s_word}, n={g.n_participants or 'NR'}"
            )

            html += f"""
<tr>
  <td style="text-align:left;color:#555;font-size:0.9em;">
    {_t('sof_critical') if out.get('importance')=='Critical' else (_t('sof_important') if out.get('importance')=='Important' else _t('sof_not_important'))}</td>
...
"""
            ...
            for f in g.all_factors:
                if f.direction != "none" and f.rationale:
                    _d_short = _t("design_rcts") if design_key=="rct" else _t("sof_obs_label")
                    footnotes.append(
                        f"<b>{oname} — {_d_short} "
    ...
    if footnotes:
        html += (f"<br><small><b>{_t('sof_explanations')}</b><br>"
